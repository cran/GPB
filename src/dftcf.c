...
//*****************************************************************************//
void gpb_dft_cf(double *res, int *nvec, int *nn, int *n, double *pp,double *avec,
double *bvec,int *funcate,double *ex,int *npp, int *wts, int *aval, int *bval, int *asum)
{
  int i,j,k,kk,m,wtsj, avalj, bvalj;
  double a2, b2, c1,c2;
  ...
  for(i=1;i<=ceil((*n)/2)+1;i++)
  {
     ...
     for(j=0;j<*npp;j++)
     {
       ...
       bvalj=bval[j];
       //Rprintf("%u,%lf \n",j, pj);
       ax=(1-pj)*cos(tt*avalj)+pj*cos(tt*bvalj);
       bx=(1-pj)*sin(tt*avalj)+pj*sin(tt*bvalj);
       tmp1=sqrt(ax*ax+bx*bx);
